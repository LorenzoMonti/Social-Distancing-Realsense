...
weightsPath = "./MobileNetSSD_deploy_pedestrian.caffemodel"
configPath =  "./MobileNetSSD_deploy_pedestrian.prototxt"
...
if __name__=='__main__':
	...
	try:
		while True:
			...
			# resize image
			frame_resized = cv2.resize(color_image,(300,300))
			...
			for i in range(detections.shape[2]):
				...
				if confidence >= 0.25: # filter prediction
					...
					if class_id == 1: # person
			            # Object location
						xLeftBottom = int(detections[0, 0, i, 3] * H)
						yLeftBottom = int(detections[0, 0, i, 4] * W)
						xRightTop   = int(detections[0, 0, i, 5] * H)
						yRightTop   = int(detections[0, 0, i, 6] * W)

						# Factor for scale to original size of frame
						heightFactor = 720/300.0
						widthFactor = 1280/300.0
						...
						centerX = (xLeftBottom + (width / 2))
						centerY = (yRightTop - (height / 2))
